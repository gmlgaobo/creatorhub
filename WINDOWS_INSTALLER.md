...
- `CreatorHubService`：后台任务队列、监控、私信接收与自动回复、公众号官方 API、HTTPS、备份和更新检查。
- `CreatorHubTray`：随当前用户登录启动，负责扫码、重新登录、打开平台后台、设备安全验证和公众号真实健康检测。
- 系统 Google Chrome Stable：安装包不内置浏览器；小红书固定使用安装主机的系统 Chrome/CDP，各账号继续使用独立 Profile。
...
- 上游的统一任务队列、账号合并、小红书私信实时监控与自动回复均由后台服务运行；桌面代理不会重复启动调度器或消息接收器。
- 小红书的登录、发布、评论、关注和私信写入固定使用系统 Chrome/CDP，并在安装主机完成可见验证。
- 非小红书账号可按主项目文档配置外部 Fingerprint Chromium 运行时，但安装包不会下载或捆绑这些内核。运行时路径必须是安装主机可访问的绝对路径，服务账户也必须具有读取和执行权限。
- 更新后如果新增配置项缺失，程序会沿用 `config.example.yaml` 中的默认值；部署前仍建议备份 `%PROGRAMDATA%\CreatorHub`。
...
- 网页、任务队列、私信规则、官方 API、草稿和素材数据可以从其他电脑操作；扫码、重新登录、设备验证、打开公众号后台和真实后台健康检测始终由安装主机的托盘程序执行，窗口只显示在安装主机。
